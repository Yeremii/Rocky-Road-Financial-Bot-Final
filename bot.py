...
# ── Webhook URLs ──────────────────────────────────────────────────────────────
# ── Data Server ──────────────────────────────────────────────────────────────
DATA_SERVER_ID      = 1480360737846071372
REPORT_CHANNEL_ID   = 1480675913912946749
...
class Database:
    ...
    async def create_report(self, reporter_id: str, reporter_name: str, title: str, description: str, image_url: str = None) -> int:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                "INSERT INTO reports (reporter_id, reporter_name, title, description, image_url, status, created_at) VALUES (?, ?, ?, ?, ?, 'Open', ?)",
                (reporter_id, reporter_name, title, description, image_url, datetime.now(timezone.utc).isoformat())
            )
            await db.commit()
            return cursor.lastrowid

    async def update_report_message_id(self, report_id: int, message_id: str):
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE reports SET report_message_id = ? WHERE id = ?", (message_id, report_id))
            await db.commit()
    ...
    async def mark_all_pending_invoices_paid(self, discord_user_id: str, verified_by: str, period: str = None, year: int = None) -> int:
...
class DashboardAPI:
    ...
    async def update_vehicle(self, request):
        try:
            ...
            DEFAULT_ITEMS = {
                "Jerky (5x)": True, "Buckshot Juices (5x)": True,
                "Repair Kits (2x)": True, "Tire Repair Kits (2x)": True,
                "Fire Extinguisher (1x)": True, "Jerry Can (1x)": True, "Traffic Cones (5x)": True
            }
            merged_inv = {**DEFAULT_ITEMS, **inventory}

            async with aiosqlite.connect(self.db.db_path) as db_conn:
                await db_conn.execute('''
                    UPDATE vehicles
                    SET current_user_id=?, current_user_name=?, inventory_json=?, missing_items=?
                    WHERE id=?
                ''', (user_id, user_name, json.dumps(merged_inv), notes, v_id))
            ...
            logger.error(f"API Error update_vehicle: {e}")
            return web.json_response({"error": str(e)}, status=500)
    ...
    async def get_leaderboard(self, request):
        try:
            ...
            # Weekly: paid invoices for current period only
            async with aiosqlite.connect(self.db.db_path) as db_conn:
                ...
                cur = await db_conn.execute('''
                    ...
                    WHERE month=? AND year=? AND status='Paid'
                      AND staff_type IN ('driver','trainer','management')
                    GROUP BY discord_username ORDER BY total_due DESC LIMIT 10
                ''', (period_id, now.year))
                weekly = [{"name": r["discord_username"], "revenue": r["total_due"]} for r in await cur.fetchall()]

            # Monthly: paid invoices this calendar month
            monthly_paid = await self.db.get_paid_this_month(now.year, now.month)
            monthly = [{"name": r["discord_username"], "revenue": r["total_due"]} for r in monthly_paid[:10]]

            return web.json_response({"weekly": weekly, "monthly": monthly})
        except Exception as e:
    ...
    async def get_totals(self, request):
        """Dashboard totals — split into expected (all) vs real (paid only)."""
        try:
            now = datetime.now(timezone.utc)
            period_id = get_period_id(now)
            ...
            async with aiosqlite.connect(self.db.db_path) as db_conn:
                base_filter = "staff_type IN ('driver','trainer','management') AND discord_user_id NOT LIKE 'MANUAL_%'"

                # Weekly expected = all invoices current period
                r = await db_conn.execute(f"SELECT COALESCE(SUM(total_due),0) FROM invoices WHERE month=? AND year=? AND {base_filter}", (period_id, now.year))
                weekly_expected = float((await r.fetchone())[0])

                # Weekly real = paid only current period
                r = await db_conn.execute(f"SELECT COALESCE(SUM(total_due),0) FROM invoices WHERE month=? AND year=? AND status='Paid' AND {base_filter}", (period_id, now.year))
                weekly_real = float((await r.fetchone())[0])

                # Monthly expected = all invoices this month
                r = await db_conn.execute(f"SELECT COALESCE(SUM(total_due),0) FROM invoices WHERE month LIKE ? AND year=? AND {base_filter}", (f'%{month_name}%', now.year))
                monthly_expected = float((await r.fetchone())[0])

                # Monthly real = paid only this month
                r = await db_conn.execute(f"SELECT COALESCE(SUM(total_due),0) FROM invoices WHERE month LIKE ? AND year=? AND status='Paid' AND {base_filter}", (f'%{month_name}%', now.year))
                monthly_real = float((await r.fetchone())[0])

                # Pending count current period
                r = await db_conn.execute(f"SELECT COUNT(*) FROM invoices WHERE month=? AND year=? AND status='Pending' AND {base_filter}", (period_id, now.year))
                pending = (await r.fetchone())[0]

                # Paid count current period
                r = await db_conn.execute(f"SELECT COUNT(*) FROM invoices WHERE month=? AND year=? AND status='Paid' AND {base_filter}", (period_id, now.year))
                paid_count = (await r.fetchone())[0]

                # Active vehicles
                r = await db_conn.execute("SELECT COUNT(*) FROM vehicles WHERE current_user_id IS NOT NULL")
                active_vehicles = (await r.fetchone())[0]

            return web.json_response({
                "period": period_id,
                "weekly_expected": weekly_expected,
                "weekly_real": weekly_real,
            ...
            return web.json_response({"weekly_expected":0,"weekly_real":0,"monthly_expected":0,"monthly_real":0,"pending_invoices":0,"paid_invoices":0,"active_vehicles":0})
...
class GoogleSheetsManager:
    ...
    def backup_spreadsheet(self, sheet_id: str, label: str = "Backup") -> dict:
        """Create a backup copy of the spreadsheet in Google Drive.
        
        Uses the Drive API v3 files.copy endpoint via gspread's authenticated session.
        gspread has no built-in .copy() method — this is the correct approach.
        """
        if not self.gc:
            return {'success': False, 'error': 'Google Sheets not initialized'}
        try:
            backup_name = label
            # Drive API v3: POST /files/{fileId}/copy
            url = f"https://www.googleapis.com/drive/v3/files/{sheet_id}/copy"
            response = self.gc.request('post', url, json={'name': backup_name})
            file_id = response.json().get('id', 'unknown')
            logger.info(f"Backup created successfully: {backup_name} (id={file_id})")
            return {'success': True, 'name': backup_name, 'id': file_id}
        except Exception as e:
...
async def on_ready():
    ...
    try:
        ...
        asyncio.ensure_future(WebhookLogger._send(WH_SYNC, {"content": f"🤖 **Bot online** — {len(synced)} commands synced — {WebhookLogger._ts()}"}))
    except Exception as e:
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    ...
    affected = await db.mark_all_pending_invoices_paid(
        payment_log['discord_user_id'], member.display_name
    )
    logger.info(f"REACTION VERIFY: user={payment_log['discord_username']} id={payment_log['discord_user_id']} affected={affected}")
    asyncio.ensure_future(WebhookLogger._send(WH_INVOICE, {"content": f"✅ **Reaction verify:** `{payment_log['discord_username']}` affected={affected} by {member.display_name} — {WebhookLogger._ts()}"}))
...
class ReportModal(discord.ui.Modal, title="Submit a Report"):
    report_title = discord.ui.TextInput(label="Title", placeholder="Short description of the issue...", max_length=100, required=True)
    description  = discord.ui.TextInput(label="Description", placeholder="Describe the problem in detail...", style=discord.TextStyle.paragraph, max_length=1000, required=True)
    image_url    = discord.ui.TextInput(label="Image URL (optional)", placeholder="https://... screenshot link", required=False, max_length=500)

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        title = self.report_title.value.strip()
        desc  = self.description.value.strip()
        img   = self.image_url.value.strip() or None

        report_id = await db.create_report(str(interaction.user.id), interaction.user.display_name, title, desc, img)
        ...
        embed.set_footer(text=f"Reporter ID: {interaction.user.id} | Status: Open")
        if img:
            embed.set_image(url=img)

        data_guild     = bot.get_guild(DATA_SERVER_ID)
        report_channel = data_guild.get_channel(REPORT_CHANNEL_ID) if data_guild else None
        view           = ReportAdminView(report_id, str(interaction.user.id), interaction.user.display_name)

        if report_channel:
            msg = await report_channel.send(embed=embed, view=view)
            await db.update_report_message_id(report_id, str(msg.id))

        confirm_desc = f"Your report **#{report_id}** has been received. You'll be notified when it's being looked at."
        confirm = discord.Embed(title="✅ Report Submitted", description=confirm_desc, color=0x57F287)
        await interaction.followup.send(embed=confirm, ephemeral=True)


class ReportAdminView(discord.ui.View):
    def __init__(self, report_id: int, reporter_id: str, reporter_name: str):
        super().__init__(timeout=None)
        self.report_id     = report_id
        self.reporter_id   = reporter_id
        self.reporter_name = reporter_name

    @discord.ui.button(label="🔄 In Progress", style=discord.ButtonStyle.primary, custom_id="report_inprogress")
    async def in_progress(self, interaction: discord.Interaction, button: discord.ui.Button):
        await db.update_report_status(self.report_id, "In Progress")
        embed = interaction.message.embeds[0]
        embed.color = discord.Color.orange()
        embed.set_footer(text=f"Reporter ID: {self.reporter_id} | Status: In Progress | By: {interaction.user.display_name}")
        button.disabled = True
        await interaction.message.edit(embed=embed, view=self)
        await interaction.response.defer()
        try:
            reporter = await bot.fetch_user(int(self.reporter_id))
            who = interaction.user.display_name
            rid = self.report_id
            dm_desc  = f"Report **#{rid}** is now in progress. **{who}** is on it - we will let you know when it is resolved."
            dm_embed = discord.Embed(title="🔄 Your Report Is Being Looked At", description=dm_desc, color=discord.Color.orange(), timestamp=datetime.now(timezone.utc))
            await reporter.send(embed=dm_embed, view=ReportContactView(interaction.user.id, interaction.user.display_name))
        except Exception:
            pass

    @discord.ui.button(label="✅ Fixed", style=discord.ButtonStyle.success, custom_id="report_fixed")
    async def fixed(self, interaction: discord.Interaction, button: discord.ui.Button):
        await db.update_report_status(self.report_id, "Fixed")
        embed = interaction.message.embeds[0]
        embed.color = discord.Color.green()
        embed.set_footer(text=f"Reporter ID: {self.reporter_id} | Status: Fixed | By: {interaction.user.display_name}")
        ...
        try:
            ...
            who = interaction.user.display_name
            rid = self.report_id
            dm_desc  = f"Your report **#{rid}** has been resolved by **{who}**. Thank you for the report!"
            dm_embed = discord.Embed(title="✅ Report Fixed", description=dm_desc, color=discord.Color.green(), timestamp=datetime.now(timezone.utc))
            await reporter.send(embed=dm_embed, view=ReportContactView(interaction.user.id, interaction.user.display_name))
        except Exception:
            pass

    @discord.ui.button(label="❌ Close", style=discord.ButtonStyle.danger, custom_id="report_close")
    async def close_report(self, interaction: discord.Interaction, button: discord.ui.Button):
        await db.update_report_status(self.report_id, "Closed")
        embed = interaction.message.embeds[0]
        embed.color = discord.Color.red()
        embed.set_footer(text=f"Reporter ID: {self.reporter_id} | Status: Closed | By: {interaction.user.display_name}")
        for item in self.children:
            item.disabled = True
        await interaction.message.edit(embed=embed, view=self)
        await interaction.response.defer()
...
class ReportContactView(discord.ui.View):
    def __init__(self, admin_id: int, admin_name: str):
        super().__init__(timeout=None)
        self.admin_id   = admin_id
        self.admin_name = admin_name

    @discord.ui.button(label="💬 Contact Admin", style=discord.ButtonStyle.secondary, custom_id="report_contact")
    async def contact_admin(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            admin = await bot.fetch_user(self.admin_id)
            admin_desc = f"**{interaction.user.display_name}** (`{interaction.user.id}`) wants to discuss their report."
            await admin.send(embed=discord.Embed(title="📩 Reporter wants to contact you", description=admin_desc, color=0x5865F2, timestamp=datetime.now(timezone.utc)))
            await interaction.response.send_message("✅ Admin notified - they will reach out via DM.", ephemeral=True)
            button.disabled = True
            await interaction.message.edit(view=self)
        except Exception:
            await interaction.response.send_message("❌ Could not contact admin right now.", ephemeral=True)

@bot.tree.command(name="report", description="Submit a bug report or issue")
